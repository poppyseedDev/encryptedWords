...
describe("FHEordle", function () {
  ...
  it("should initialize encrypted word ID", async function () {
    ...
    const encryptedWordId = this.instances.bob.encrypt16(3); // Encrypt a test ID

    // Interact with the contract
    const tx = await this.contract.connect(bob).initialize(this.signers.alice.address, bob.address, encryptedWordId);
    await tx.wait();

    const word1Id = await this.contract.getWord1Id();
